...
export function StaffPage() {
  ...
  const handleToggleStatus = async (user: ClientUser) => {
    ...
    const actionText = nextStatus === 'active' ? 'تفعيل' : 'تعطيل';

    try {
      await toggleClientUserStatus(user.id, client.id, nextStatus);
      toast.success(`تم ${actionText} حساب ${user.name} بنجاح`);
      loadUsers();
    } catch (err: any) {
            ...
            <div className="p-6 overflow-y-auto space-y-6 text-xs flex-1">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-slate-50 p-4 rounded-xl border border-slate-200">
                ...
                <div>
                  ...
                  <span className={`font-bold mt-0.5 block ${selectedUser.status === 'active' ? 'text-emerald-700' : 'text-slate-500'}`}>
              ...
              {/* Resolved Permissions Matrix Display */}
              <div>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-1.5 font-bold text-slate-900 text-sm">
                    <Shield className="h-4 w-4 text-indigo-600" />
                    <span>مصفوفة الصلاحيات الممنوحة (Permission Matrix)</span>
                  </div>
                  <span className="text-slate-400 text-[11px]">
                    {selectedUser.role === 'owner' ? 'صلاحيات كاملة شاملة' : 'محددة حسب الدور الافتراضي'}
                  </span>
                </div>

                <div className="border border-slate-200 rounded-xl divide-y divide-slate-100 overflow-hidden">
                  {PERMISSION_MODULES.map((mod) => {
                    const permissions = resolveUserPermissions(selectedUser.role, selectedUser.custom_permissions);
                    const isAllAllowed = permissions.has('*') || permissions.has(`${mod.id}.*`);
                    ...
                    return (
                      <div key={mod.id} className="p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2 hover:bg-slate-50/50">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5">
                          {mod.actions.map((act) => {
                            const isActionAllowed = isAllAllowed || permissions.has(`${mod.id}.${act}`);
